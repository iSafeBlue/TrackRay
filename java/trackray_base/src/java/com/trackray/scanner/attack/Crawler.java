...
import com.trackray.scanner.enums.*;
...
public class Crawler {
    ...
    public Crawler(CrawlerPage crawlerPage) {
        ...
        plugins = ContextLoader.getCurrentWebApplicationContext().getBeansOfType(CrawlerPlugin.class);
        {
            String reg = ".+";
            String exp = "";
            for (String s : suffix) {
    ...
    private void processPlugin(CrawlerPage crawlerPage) {
        for (Map.Entry<String, CrawlerPlugin> entry : plugins.entrySet()) {
            CrawlerPlugin plugin = null;
            try {
                plugin = entry.getValue().getClass().newInstance();
    ...
    private void processXSS(CrawlerPage crawlerPage) {
        ...
        if (url.matches(Constant.Vuln.FILE_READ_VULN_REGEX)){
            Vulnerable build = new Vulnerable.VulnBuilder()
                    .payload(url).message("该链接可能会存在文件读取/包含等漏洞")
                    .vulnType(VulnType.FILE_OPERATION).level(VulnLevel.WARNING)
                    .build();
        ...
        for (String payload : Payload.xssPayload) {
            for (int i=0;i<keys.size();i++){
                ...
                if (content.contains(payload)){
                    ...
                    Vulnerable build = new Vulnerable.VulnBuilder().message("通过爬虫检测出来该链接存在XSS漏洞 param=" + key).payload(path.toString())
                            .vulnType(VulnType.XSS).level(VulnLevel.DANGER).build();
